...
from forms.utils.exceptions import FormulaStringSyntaxErrorException, FormulaStringUnsupportedException
...
def build_from_subexpression(tokens, pos: int) -> (PlanNode, int):
    ...
    while cur_pos < len(tokens):
        ...
        # detect syntax error
        if cur_token.type in subexpression_tokens or cur_token.type in op_in_post_tokens:
            detect_syntax_error(cur_token, last_token, cur_pos)
            last_token = cur_token

        # parse tokens based on the type
        if cur_token.type in subexpression_tokens:
            new_plan_node = None
            if cur_token.type == Token.FUNC:
                if not cur_token.subtype == Token.OPEN:
            ...
            build_from_op_in_post_subexpression(tokens, cur_pos, cur_plan_node)
...
def build_from_op_pre_subexpression(tokens, pos: int) -> (PlanNode, int):
    ...
    plan_node = FunctionNode(from_function_str(cur_token.value), formula_apply_axis)
...
def build_from_op_in_post_subexpression(tokens, pos: int, child_plan_node: PlanNode) -> (PlanNode, int):
    ...
    cur_plan_node = FunctionNode(from_function_str(cur_token.value), formula_apply_axis)
...
def detect_syntax_error(cur_token, last_token, cur_pos):
    if cur_token.type in subexpression_tokens and (
        last_token.type in subexpression_tokens or last_token.type == Token.OP_POST
    ):
        raise_syntax_exception(cur_pos, cur_token)
        ...
        raise_syntax_exception(cur_pos, cur_token)
    elif cur_token.value == Token.OP_POST and last_token.type not in subexpression_tokens:
        raise_syntax_exception(cur_pos, cur_token)
