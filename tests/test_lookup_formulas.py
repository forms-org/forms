...
def test_compute_vlookup_exact():
    ...
    expected_df = pd.DataFrame(np.tile([1.6222], DF_ROWS))
    assert np.allclose(computed_df.values, expected_df.values, atol=1e-03)
    computed_df = forms.compute_formula(df, f"=SUM(VLOOKUP(1, C1:I{DF_ROWS}, 5, 0), VLOOKUP(1, C1:I{DF_ROWS}, 5, 0))")
    expected_df = pd.DataFrame(np.tile([3.2444], DF_ROWS))
    assert np.allclose(computed_df.values, expected_df.values, atol=1e-03)
    computed_df = forms.compute_formula(df, f"=VLOOKUP(1.5, C1:I{DF_ROWS}, 5, FALSE)")
    assert computed_df.iloc[:, 0].isnull().all()
    computed_df = forms.compute_formula(df, f'=VLOOKUP(M1, M1:O{DF_ROWS}, 2, 0)').astype(int)
    expected_df = pd.DataFrame(np.arange(DF_ROWS - 1, -1, -1))
    assert np.allclose(computed_df.values, expected_df.values, atol=1e-03)
...
def test_compute_vlookup_approx():
    ...
    assert computed_df.iloc[:, 0].isnull().all()
    computed_df = forms.compute_formula(df, f"=VLOOKUP(C1, C1:I{DF_ROWS}, 5)")
    expected_df = pd.DataFrame(np.tile([0.4111, 1.6222, 2.93333333, 3.999], (DF_ROWS // 4)))
    assert np.allclose(computed_df.values, expected_df.values, atol=1e-03)
    computed_df = forms.compute_formula(df, f"=VLOOKUP(N1, C1:I{DF_ROWS}, 5)")
    expected_df = pd.DataFrame(np.tile([3.999, 2.93333333, 1.6222, 0.4111], (DF_ROWS // 4)))
    assert np.allclose(computed_df.values, expected_df.values, atol=1e-03)
    ...
    expected_df = pd.DataFrame(np.tile([DF_ROWS - 1], DF_ROWS))
    assert np.allclose(computed_df.values, expected_df.values, atol=1e-03)
